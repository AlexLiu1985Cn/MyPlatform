 ...
 *
 */
public abstract class ListCommContainer extends LinearLayout {
	/**
	 * 代表插入头部
	 */
	...
	public ListCommContainer(Context context) {
		super(context);
		init();
	}

	public ListCommContainer(Context context, AttributeSet attrs) {
		super(context, attrs);
		init();
	}
	/**
	...
	private void createChildrenView(int count) {
		...
		for (int i = 0; i < count; i++) {
			...
			if(index%2 == 1){
				itemView.setBackgroundResource(E_color.e_bedaf2);
			}else{
				itemView.setBackgroundResource(E_color.e_d5e6f5);
			}
			if (index == 0) {
				LayoutParams params = new LayoutParams(LayoutParams.MATCH_PARENT, getResources().getDimensionPixelSize(E_dimens.e_size_100));
				params.setMargins(getResources().getDimensionPixelSize(E_dimens.e_size_24), getResources().getDimensionPixelSize(E_dimens.e_size_10), getResources().getDimensionPixelSize(E_dimens.e_size_24), 0);
				itemView.setLayoutParams(params);
			} else {
				LayoutParams params = new LayoutParams(LayoutParams.MATCH_PARENT, getResources().getDimensionPixelSize(E_dimens.e_size_100));
				params.setMargins(getResources().getDimensionPixelSize(E_dimens.e_size_24), getResources().getDimensionPixelSize(E_dimens.e_size_5), getResources().getDimensionPixelSize(E_dimens.e_size_24), 0);
				itemView.setLayoutParams(params);
			}
